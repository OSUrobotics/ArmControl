...
public:
  ...
  bool comparePoses(geometry_msgs::Pose first, geometry_msgs::Pose second, float precision);
  void closeGripper(geometry_msgs::Pose start_pose);
  void openGripper(geometry_msgs::Pose start_pose);
  void verifyExecution(geometry_msgs::Pose target, float precision, bool execute);
  bool validatePlan(moveit_msgs::RobotTrajectory tr, int treshhold);
  std::vector<geometry_msgs::Pose> computePoints(geometry_msgs::Pose start_pose, geometry_msgs::Pose end_pose, int numPoints);
};